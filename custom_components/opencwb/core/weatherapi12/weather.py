...
from .uris import ICONS_BASE_URI


class Weather:
    """
    A class encapsulating raw weather data.
    A reference about OCWB weather codes and icons can be found at:
    ...
    :param weather_code: OCWB weather condition code
    ...
    def temperature(self, unit='celsius'):
        ...
        :param unit: the unit of measure for the temperature values. May be:
            '*kelvin*', '*celsius*' (default) or '*fahrenheit*'
        :type unit: str
    ...
    def from_dict_of_lists(cls, the_dict):
        ...
        if 'message' in the_dict and 'cod' in the_dict:
            if the_dict['cod'] == "404":
                print("OCWB API: data not found - response payload: " + \
                      json.dumps(the_dict))
                return None
            elif the_dict['cod'] != "200":
                raise exceptions.APIResponseError(
                    "OCWB API: error - response payload: " + json.dumps(the_dict), the_dict['cod'])
