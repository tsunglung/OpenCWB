...
class StationHistory:
    ...
    the OCWB Weather API: ticks (one data chunk per minute) data, hourly and daily
    data.
    ...
    :type interval: str
    :param reception_time: GMT UNIXtime of the data reception from the OCWB web
         API
    ...
    def reception_time(self, timeformat='unix'):
        """Returns the GMT time telling when the meteostation history data was
           received from the OCWB Weather API
    ...
    def from_dict(cls, d):
        ...
        # of HTTP error status codes by the OCWB API but it's supposed to be
        ...
        # case the OCWB API responses are the very same either when no results
        # are found for a station and when the station does not exist!
        measurements = {}
        try:
            if 'cod' in d and d['cod'] != "200":
                raise exceptions.APIResponseError(
                                          "OCWB API: error - response payload: " + str(d), d['cod'])
            if str(d['cnt']) == "0":
                return None
            else:
                for item in d['list']:
